export class GeminiIntegration {
    ...
    async checkConnection() {
        ...
        try {
            ...
            if (this.debug)
                console.log('Gemini not available:', error);
            return false;
        }
    }
    ...
    async processCodeRequest(code, request, language = 'javascript') {
...
Please analyze the code and provide a helpful response.`;
    ...
    async processResearchRequest(query, useWebSearch = true) {
        let context = '';
        if (useWebSearch && this.serpApiKey) {
            try {
                ...
                context = `Recent web search results for "${query}":\n\n` +
                    searchResults.map(result => `${result.position}. ${result.title}\n${result.snippet}\nSource: ${result.link}`).join('\n\n') + '\n\n';
            }
            catch (error) {
                if (this.debug)
                    console.log('Web search failed, proceeding without:', error);
            }
        }
...
You provide thoughtful, comprehensive responses while maintaining a professional yet approachable tone.`;
    ...
    async analyzeMemoryForInsights(memories) {
        const memoryContext = memories.slice(0, 10).map(m => `[${m.type}] ${m.content} (importance: ${m.importance})`).join('\n');
