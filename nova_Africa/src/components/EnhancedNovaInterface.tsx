...
import { 
  ...
  MessageSquare, Lightbulb, Download, Clock, Layers, Sparkles
} from 'lucide-react';
...
const EnhancedNovaInterface: React.FC = () => {
  ...
  useEffect(() => {
    ...
    nova.on('statusUpdated', (status: unknown) => {
      setSystemStatus(status as NovaSystemStatus);
    });

    nova.on('taskCreated', (task: unknown) => {
      setTasks(prev => [...prev, task as NovaTask]);
    });

    nova.on('taskUpdated', (task: unknown) => {
      setTasks(prev => prev.map(t => t.id === (task as NovaTask).id ? task as NovaTask : t));
    });
  ...
  const addSystemMessage = (content: string) => {
    const message: ChatMessage = {
      id: `system-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${messages.length}`,
      content,
      sender: 'system',
      timestamp: Date.now(),
      type: 'message',
  ...
  const addUserMessage = (content: string) => {
    const message: ChatMessage = {
      id: `user-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${messages.length}`,
      content,
      sender: 'user',
      timestamp: Date.now(),
      type: 'message',
  ...
  const addNovaMessage = (content: string, type: 'message' | 'code' | 'command' | 'result' = 'message') => {
    const message: ChatMessage = {
      id: `nova-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${messages.length}`,
      content,
      sender: 'nova',
      timestamp: Date.now(),
      type,
  ...
  const renderMemoryTab = () => (
    <div className="p-6 space-y-6">
      <div className="bg-gray-800 p-6 rounded-lg">
        <h3 className="text-lg font-semibold mb-4 flex items-center">
          <Brain className="w-5 h-5 mr-2" />
          NOVA's Memory & Autonomous Thinking
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-700 p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-blue-400">{nova.allMemories.length}</span>
            </div>
            <div className="text-xs text-gray-500">Stored across sessions</div>
          </div>

          <div className="bg-gray-700 p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-400">Autonomous Thoughts</span>
              <span className="text-sm text-purple-400">
                {nova.getMemory({ type: 'thought' }).length}
              </span>
            </div>
            <div className="text-xs text-gray-500">AI-generated insights</div>
          </div>

          <div className="bg-gray-700 p-4 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-400">Learning Events</span>
              <span className="text-sm text-green-400">
                {nova.getMemory({ type: 'learning' }).length}
              </span>
            </div>
        ...
        <div className="space-y-4">
          <div>
            <h4 className="text-md font-medium mb-3 text-purple-400">🤔 Recent Autonomous Thoughts</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {nova.getMemory({ type: 'thought', limit: 5 }).map((memory) => (
                <div key={memory.id} className="bg-gray-700 p-3 rounded text-sm">
              ...
              {nova.getMemory({ type: 'thought' }).length === 0 && (
                <div className="text-gray-500 text-sm">No autonomous thoughts yet. Start a conversation!</div>
              )}
            </div>
          </div>

          <div>
            <h4 className="text-md font-medium mb-3 text-green-400">🧠 Learning & Patterns</h4>
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {nova.getMemory({ type: 'learning', limit: 5 }).map((memory) => (
                <div key={memory.id} className="bg-gray-700 p-3 rounded text-sm">
                  <div className="text-gray-300">{memory.content}</div>
                  <div className="text-xs text-gray-500 mt-1">
                    {new Date(memory.timestamp).toLocaleString()}
              ...
              {nova.getMemory({ type: 'learning' }).length === 0 && (
                <div className="text-gray-500 text-sm">No learning events yet. Continue the conversation!</div>
              )}
            </div>
          </div>

          <div>
            <h4 className="text-md font-medium mb-3 text-blue-400">💬 Conversation Context</h4>
            <div className="bg-gray-700 p-3 rounded text-sm">
              <div className="text-gray-300 font-mono text-xs">
                {nova.getConversationContext(3) || 'No recent conversation context available.'}
              </div>
            </div>
          </div>
        </div>
